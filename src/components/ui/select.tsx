...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        // Mirrors the Button `outline` variant so triggers sit next to buttons
        // in a filter bar without visual drift. Tokens only — no raw values.
        "flex w-fit items-center justify-between gap-2 rounded-lg border border-input bg-input/30 px-2.5 text-sm whitespace-nowrap text-foreground transition-[color,box-shadow] outline-none data-[size=default]:h-9 data-[size=sm]:h-8 hover:bg-input/50 focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 disabled:cursor-not-allowed disabled:opacity-50 data-placeholder:text-muted-foreground aria-invalid:border-destructive [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 [&_svg]:text-muted-foreground",
        className
      )}
      {...props}
    >
